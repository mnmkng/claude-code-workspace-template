...
| From | To |
|---|---|
| `WORKSPACE_*` environment variables and constants | `WORKSPACE_*` (and `CLAUDE_WORKSPACE_ROOT` to `CLAUDE_WORKSPACE_ROOT`) |
| `Workspace security config` (the banner the security rule keys on) | `Workspace security config` |
| `/opt/claude-workspace/composed` staging directory | `/opt/claude-workspace/composed` |
| `WORKSPACE-BOOTSTRAP`, `WORKSPACE-CLAUDE-CODE-` marker comments | `WORKSPACE-BOOTSTRAP`, `WORKSPACE-CLAUDE-CODE-` |
| Root sentinel `--team root` | `--team root` |
| `find_workspace_root`, `workspace_root` identifiers | `find_workspace_root`, `workspace_root` |
| Repo name and setup-script path | `<your-org>/<your-repo>`, `/home/user/<your-repo>` |
| Prose: "workspace root", "workspace tree", company name in docstrings and test fixtures | workspace root, workspace tree, generic wording |
...
The workspace's own PreToolUse hook refuses to let Claude modify any file whose path ends in a protected name, wherever it lives, copies included. So the nine protected files were carried over byte-identical, and the de-branding of the six that mention the source company (plus the removal of the source company's MCP allow entries and MCP keys from the settings file) was written as a proposed diff, `protected-files.patch` at the repo root, for a human to apply once:
...
For a refresh, the equivalent substitutions as a one-liner:
...
sed -i -E \
  -e 's#/opt/claude-workspace/composed#/opt/claude-workspace/composed#g' \
  -e 's/WORKSPACE-BOOTSTRAP/WORKSPACE-BOOTSTRAP/g' \
  -e 's/Workspace security config/Workspace security config/g' \
  -e 's/CLAUDE_WORKSPACE_ROOT/CLAUDE_WORKSPACE_ROOT/g' \
  -e 's/\bAPIFY_/WORKSPACE_/g' \
  -e 's/find_workspace_root/find_workspace_root/g' \
  -e 's/\bAPIFY_ROOT\b/WORKSPACE_ROOT/g' \
  -e "s#bash Apify/scripts/install.sh#bash <your-repo>/scripts/install.sh#g" \
  -e 's/workspace root/workspace root/g; s/workspace tree/workspace tree/g' \
  -e 's/an workspace tree/a workspace tree/g; s/any workspace tree/any workspace tree/g' \
  -e 's#Apify/\.claude/settings\.json#<workspace>/.claude/settings.json#g' \
  -e 's/Claude Code workspace/Claude Code workspace/g' \
  -e 's/the workspace root/the workspace root/g; s/Apify workspace/workspace/g' \
  -e 's/workspace claude wrapper/workspace claude wrapper/g' \
  $files
