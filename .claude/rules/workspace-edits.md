...
This rule governs files that describe reality. While this repo still carries the template's fabricated example company, that content's headers are specimen values with no source to check against - see CONTRIBUTING.md, "The example company". The exemption is narrow and self-terminating: it covers only the example content shipped with the template, and it ends the moment `/setup-workspace` writes a real company's. Never reason from it that some other file is "example content".
